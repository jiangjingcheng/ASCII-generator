...
def get_args():
    ...
    parser.add_argument("--output", type=str, default="data/ouput_video_overlay.mp4", help="Path to output video")
...
def main(opt):
    ...
    while cap.isOpened():
        ...
        # 通过 try-except 结构检查是否已经创建了 out 对象，如果没有创建，则初始化 cv2.VideoWriter 来准备将视频帧写入文件。
        try:
            out
        except:
